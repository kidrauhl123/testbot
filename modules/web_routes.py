...
def register_routes(app, notification_queue):
    ...
    @app.route('/api/today-stats-by-submitter')
    @login_required
    def today_stats_by_submitter():
        """获取按不同网页提交者分组的今日有效订单数"""
        try:
            from datetime import datetime
            import pytz
            ...
            logger.info(f"查询今日({today})按提交者分组的有效订单...")
            
            # 根据数据库类型构建查询
            if DATABASE_URL.startswith('postgres'):
                query = """
                    SELECT 
                        COALESCE(web_user_id, '未知') as submitter,
                        COUNT(*) as count
                    FROM orders 
                    WHERE (
                        -- 充值成功且非长时间未收到
            ...
            logger.info(f"按提交者分组的今日有效订单数: {stats}")
            ...
            logger.error(f"获取按提交者分组的今日统计失败: {str(e)}", exc_info=True)
            return jsonify({'success': False, 'error': str(e)}), 500
