...
async def on_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect("orders.db")
    c = conn.cursor()
    c.execute("UPDATE orders SET accepted_by_username=? WHERE id=?", (username, oid))
    conn.commit()
    conn.close()
...
async def show_personal_stats(query, user_id, date_str, period_text):
    ...
    c.execute("""SELECT package, 
                 ...
                 GROUP BY package""",
              (STATUS['COMPLETED'], date_str, str(user_id), STATUS['ACCEPTED'], STATUS['COMPLETED'], STATUS['FAILED']))
    rows = c.fetchall()
    conn.close()
    ...
    for pkg, accepted_count, completed_count in rows:
        # 只有完成的订单才计入薪资
...
async def show_period_stats(query, user_id, start_date, end_date, period_text):
    ...
    # 查询时间段数据，区分接单总数和完成数
    if user_id == 'all':
        c.execute("""SELECT accepted_by, package, 
                            COUNT(*) as total_accepted,
                            SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) as completed_count
                     FROM orders
        ...
        c.execute("""SELECT package, 
                     ...
                     GROUP BY package""",
                  (STATUS['COMPLETED'], start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"),
                   str(user_id), STATUS['ACCEPTED'], STATUS['COMPLETED'], STATUS['FAILED']))
    ...
    if user_id == 'all':
        ...
        for uid, pkg, accepted_count, completed_count in rows:
...
async def show_all_stats(query, date_str, period_text):
    ...
    c.execute("""SELECT accepted_by, package, 
                 ...
                 GROUP BY accepted_by, package""",
              (STATUS['COMPLETED'], date_str, STATUS['ACCEPTED'], STATUS['COMPLETED'], STATUS['FAILED']))
    rows = c.fetchall()
    conn.close()
